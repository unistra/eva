...
class CustomFileInput(ClearableFileInput):
    ...
    def render(self, name, value, attrs=None):
        ...
        if self.is_initial(value):
            ...
            substitutions['add_pdf'] = '<button id="add_pdf" class="  \
            pull-right">%s</button></br>' % _('Ajouter le document')
        return (template % substitutions)

    template_with_clear = (
        '<button id="delete_pdf" class=" has-1em-margin-top"> \
        %s</button></br>' % _('Supprimer le document')
    )
